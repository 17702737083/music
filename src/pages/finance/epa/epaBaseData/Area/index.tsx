...
import { AreaSource, OptionsData } from '../../data';
import { Button, message, Upload } from 'antd';
import Icon, { PlusOutlined } from '@ant-design/icons';
import * as xlsx from 'xlsx';
import { useModel } from 'umi';
import { baseDownloadTemplateArea, findAllArea, saveAllArea, saveArea } from './service';
...
const Area: React.FC = () => {
    ...
    useEffect(async () => {
        ...
        const reslult = await findAllArea(initialState?.currentUser?.employeeInfoDTO?.emplid?initialState?.currentUser?.employeeInfoDTO?.emplid:'',{} as AreaSource);
    ...
    const uploadprops = {
        ...
        beforeUpload: (file: Blob, fileList: any) => {
            ...
            fileReader.onload = async event => {
                ...
                console.log(data2);
                if (data.length > 0) {
                    const result = await saveAllArea(company,data2 as AreaSource[]);
                    if (result.code == 200) {
    ...
    const columns: ProColumns<AreaSource>[] = [
        {
            title: '公司别',
    ...
    const downloadtemplate = () => {
        baseDownloadTemplateArea("Area");
    }
    //下载表格数据
    const downloadData = () => {
        const aoa = [
            ["序号", "公司别", "区域代码", "区域名称", "区域别", "是否有效"]
        ]
        ...
        <PageContainer>
            <ProTable<AreaSource>
                ...
                toolBarRender={() => [
                    ...
                    <ModalForm<AreaSource>
                        ...
                        onFinish={async (values) => {
                            ...
                            const result = await saveArea(values as AreaSource);
                            if (result.code == 200) {
                    ...
                    >
                        <ProForm.Group>
                            <ProFormText
                                ...
                                label="区域代码"
                            />
                            <ProFormText width="sm" name="areaName" label="区域名称" />
                            <ProFormText width="sm" name="areaType" label="区域别" />
                            <ProFormSelect
                                options={[
                                    {
                                        value: 'Y',
                ...
                request={async (params = {}, sort, filter) => {
                    ...
                    const result: any =await findAllArea(initialState?.currentUser?.employeeInfoDTO?.emplid?initialState?.currentUser?.employeeInfoDTO?.emplid:'',params as AreaSource);
                ...
                editable={{
                    ...
                    onSave: async (key, row) => {
                        const result = await saveArea(row as AreaSource);
