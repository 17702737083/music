...
import { baseDownloadTemplateCurrency, findAllCurrency, saveAllCurrency, saveCurrency } from './service';
...
const Currency: React.FC = () => {
    ...
    useEffect(async () => {
        ...
        const reslult = await findAllCurrency({} as CurrencySource);
    ...
    const uploadprops = {
        ...
        beforeUpload: (file: Blob, fileList: any) => {
            ...
            fileReader.onload = async event => {
                ...
                console.log(data2);
                if (data.length > 0) {
                    const result = await saveAllCurrency(data2 as CurrencySource[]);
                    if (result.code == 200) {
    ...
    const columns: ProColumns<CurrencySource>[] = [
        {
            title: '币别',
            dataIndex: 'currency',
            width: 120,
    ...
    const downloadtemplate = () => {
        baseDownloadTemplateCurrency("Currency");
    }
    //下载表格数据
    const downloadData = () => {
        const aoa = [
        ...
        xlsx.writeFile(wb, '币别.xlsx')
    }
    return (
        <PageContainer>
            <ProTable<CurrencySource>
                rowKey="id"
                actionRef={actionRef}
                // scroll={{ x: 2000 }}
                // buttonText="Download as XLS"
                toolBarRender={() => [
                    <ModalForm<CurrencySource>
                        title="新建"
                        trigger={
                            <Button type="primary">
                                <PlusOutlined />
                        ...
                        onFinish={async (values) => {
                            ...
                            const result = await saveCurrency(values as CurrencySource);
                            if (result.code == 200) {
                    ...
                    >
                        <ProForm.Group>
                            <ProFormText
                                ...
                                label="币别"
                            />
                            <ProFormText width="sm" name="currencyName" label="币别名称" />
                            <ProFormSelect
                                options={[
                                    {
                                        value: 'Y',
                ...
                request={async (params = {}, sort, filter) => {
                    ...
                    const result: any =await findAllCurrency(params as CurrencySource);
                ...
                editable={{
                    ...
                    onSave: async (key, row) => {
                        const result = await saveCurrency(row as CurrencySource);
