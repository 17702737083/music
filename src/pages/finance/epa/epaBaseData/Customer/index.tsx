...
import { downTemplateCustomer, findAllCustomer, saveAllCustomer, saveCustomer } from './service';
import { Button, message, Upload } from 'antd';
import { CustomerSourceType, OptionsData } from '../../data';
...
const Customer: React.FC = () => {
    ...
    const [dataSource, setDataSource] = useState<CustomerSourceType[]>([]);
    ...
    useEffect(async () => {
        ...
        const reslult = await findAllCustomer(initialState?.currentUser?.employeeInfoDTO?.emplid?initialState?.currentUser?.employeeInfoDTO?.emplid:'',{} as CustomerSourceType);
    ...
    const uploadprops = {
        ...
        beforeUpload: (file: Blob, fileList: any) => {
            ...
            fileReader.onload = async event => {
                ...
                console.log(data2);
                if (data.length > 0) {
                    const result = await saveAllCustomer(company,data2 as CustomerSourceType[]);
                    if (result.code == 200) {
    ...
    const columns: ProColumns<CustomerSourceType>[] = [
        {
            title: '公司别',
    ...
    const downloadtemplate = () => {
        downTemplateCustomer("customer");
    }
    //下载表格数据
    const downloadData = () => {
        const aoa = [
            ['序号', '公司别', '客户代码', '客户名称', '是否有效']
        ]
        ...
        xlsx.writeFile(wb, '客户.xlsx')
    }
    return (
        <PageContainer>
            <ProTable<CustomerSourceType>
                rowKey="id"
                actionRef={actionRef}
                scroll={{ x: 1000 }}
                toolBarRender={() => [
                    ...
                    <div style={{ marginBottom: 25 }}>
                        <ModalForm<CustomerSourceType>
                            ...
                            onFinish={async (values) => {
                                ...
                                const result = await saveCustomer(values as CustomerSourceType);
                                if (result.code == 200) {
                        ...
                        >
                            <ProForm.Group>
                                <ProFormText
                                    ...
                                    label="公司别"
                                />
                                <ProFormText width="sm" name="customerId" label="客户代码" />
                                <ProFormText width="sm" name="customerName" label="客户名称" />
                                <ProFormSelect
                                    options={[
                                        {
                                            value: 'Y',
                ...
                request={async (params = {}, sort, filter) => {
                    ...
                    const result:any=await findAllCustomer(initialState?.currentUser?.employeeInfoDTO?.emplid?initialState?.currentUser?.employeeInfoDTO?.emplid:'',params as CustomerSourceType);
                ...
                editable={{
                    ...
                    onSave: async (key, row) => {
                        const result = await saveCustomer(row as CustomerSourceType);
