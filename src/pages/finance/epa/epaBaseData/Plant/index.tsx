...
import { baseDownloadTemplatePlant, findAllPlant, saveAllPlant, savePlant } from './service';
...
const Plant: React.FC = () => {
    ...
    useEffect(async () => {
        ...
        const reslult = await findAllPlant({} as PlantSource);
    ...
    const uploadprops = {
        ...
        beforeUpload: (file: Blob, fileList: any) => {
            ...
            fileReader.onload = async event => {
                ...
                console.log(data2);
                if (data.length > 0) {
                    const result = await saveAllPlant(data2 as PlantSource[]);
                    if (result.code == 200) {
    ...
    const columns: ProColumns<PlantSource>[] = [
        {
            title: '工厂代码',
            dataIndex: 'plantCode',
            width: 120,
    ...
    const downloadtemplate = () => {
        baseDownloadTemplatePlant("Plant");
    }
    //下载表格数据
    const downloadData = () => {
        const aoa = [
            ["序号", "工厂代码", "工厂名称", "所属集团", "是否有效"]
        ]
        ...
        xlsx.writeFile(wb, '厂别.xlsx')
    }
    return (
        <PageContainer>
            <ProTable<PlantSource>
                rowKey="id"
                actionRef={actionRef}
                // scroll={{ x: 2000 }}
                // buttonText="Download as XLS"
                toolBarRender={() => [
                    <ModalForm<PlantSource>
                        title="新建"
                        trigger={
                            <Button type="primary">
                                <PlusOutlined />
                        ...
                        onFinish={async (values) => {
                            ...
                            const result = await savePlant(values as PlantSource);
                            if (result.code == 200) {
                    ...
                    >
                        <ProForm.Group>
                            <ProFormText
                                ...
                                label="工厂代码"
                            />
                            <ProFormText width="sm" name="plantName" label="工厂名称" />
                            <ProFormText width="sm" name="companyCode" label="所属集团" />
                            <ProFormSelect
                                options={[
                                    {
                                        value: 'Y',
                ...
                request={async (params = {}, sort, filter) => {
                    ...
                    const result:any=await findAllPlant(params as PlantSource);
                ...
                editable={{
                    ...
                    onSave: async (key, row) => {
                        const result = await savePlant(row as PlantSource);
