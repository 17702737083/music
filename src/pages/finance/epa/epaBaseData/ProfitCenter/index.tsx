...
import { baseDownloadTemplateProfitCenter, findAllProfitCenter, saveAllProfitCenter, saveProfitCenter } from './service';
...
const ProfitCenter: React.FC = () => {
    ...
    useEffect(async () => {
        ...
        const reslult = await findAllProfitCenter({} as ProfitCenterSource);
    ...
    const uploadprops = {
        ...
        beforeUpload: (file: Blob, fileList: any) => {
            ...
            fileReader.onload = async event => {
                ...
                console.log(data2);
                if (data.length > 0) {
                    const result = await saveAllProfitCenter(data2 as ProfitCenterSource[]);
                    if (result.code == 200) {
    ...
    const columns: ProColumns<ProfitCenterSource>[] = [
        {
            title: 'Profit Center',
    ...
    const downloadtemplate = () => {
        baseDownloadTemplateProfitCenter("ProfitCenter");
    }
    //下载表格数据
    const downloadData = () => {
        const aoa = [
            ["序号", "Profit Center", "BU2", "Customer Group", "BPMA", "是否有效"]
        ]
        ...
        xlsx.writeFile(wb, 'Profit Center.xlsx')
    }
    return (
        <PageContainer>
            <ProTable<ProfitCenterSource>
                rowKey="id"
                actionRef={actionRef}
                scroll={{ x: 1000 }}
                // buttonText="Download as XLS"
                toolBarRender={() => [
                    <ModalForm<ProfitCenterSource>
                        title="新建"
                        trigger={
                            <Button type="primary">
                                <PlusOutlined />
                        ...
                        onFinish={async (values) => {
                            ...
                            const result = await saveProfitCenter(values as ProfitCenterSource);
                            if (result.code == 200) {
                    ...
                    >
                        <ProForm.Group>
                            <ProFormText
                                ...
                                label="Profit Center"
                            />
                            <ProFormText width="sm" name="bu2" label="BU2" />
                            <ProFormText width="sm" name="customerGroup" label="Customer Group" />
                            <ProFormText width="sm" name="bpma" label="BPMA" />
                            <ProFormSelect
                                options={[
                                    {
                                        value: 'Y',
                ...
                request={async (params = {}, sort, filter) => {
                    ...
                    const result:any=await findAllProfitCenter(params as ProfitCenterSource);
                ...
                editable={{
                    ...
                    onSave: async (key, row) => {
                        const result = await saveProfitCenter(row as ProfitCenterSource);
